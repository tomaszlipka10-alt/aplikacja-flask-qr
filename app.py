...
# Init DB
def init_db():
    """Create tables and seed an admin user.

    Render can keep a persistent disk or reuse an old SQLite file. If the DB schema
    is older than the current models (e.g. missing columns), the app would crash
    on startup. For this demo app, we reset the DB in that situation.
    """
    with app.app_context():
        try:
            db.create_all()
            # Sanity query: fails if schema is stale (e.g. missing columns)
...
# On Render we want DB ready before the first request.
