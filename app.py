...
def api_products():
    ...
    if product:
        ...
        db.session.commit()
        return jsonify({"message": "Updated", "id": product.id, "updated": True})

    product = Product(item_number=item_number, name=name, location_name=location, current_stock=0)
    db.session.add(product)
    db.session.commit()
    return jsonify({"message": "Created", "id": product.id, "updated": False}), 201
